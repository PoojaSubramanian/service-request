...
class PostsIndex extends React.Component {
  ...
  renderPosts() {
    if (this.props.post === null) {
      return <p>"Add a service request!"</p>;
    }
            ...
            message={
              <div id={post.subject}>
                ...
                <i>
                  Status : {post.status}{" "}
                  {post.status !== open &&
                    post.fulfillerName !== null &&
                    "by " + post.fulfillerName}
                </i>
                <p style={{ marginRight: 2.5 + "em" }}>
                  <Link to={`/posts/${post._id}`}>
      ...
      <div className={this.props.classes.root}>
        <Paper>
          <Typography variant="h4" color="#ad9c78">
            ...
            {this.props.userState ? this.props.userState : ""} {this.props.zip}
          </Typography>
          <MenuList>
            <MenuItem className={this.props.classes.menuItem}>
              <ListItem>
                <Link to="/posts/new">Add New Request</Link>
              </ListItem>
            </MenuItem>
            <MenuItem className={this.props.classes.menuItem}>
              <ListItem button onClick={this.onClickingHome.bind(this)}>
                Feed
              </ListItem>{" "}
            </MenuItem>
            <MenuItem className={this.props.classes.menuItem}>
              <ListItem button onClick={this.onGetServiceByUserId.bind(this)}>
                My Needs
              </ListItem>
            </MenuItem>
            <MenuItem className={this.props.classes.menuItem}>
              <ListItem
                button
                onClick={this.onGetServiceByFulfillerId.bind(this)}
              >
                My Todos
              </ListItem>
            </MenuItem>
...
export default connect(
  mapStateToProps,
  {
    ...
    getServiceByFulfillerId: getServiceByFulfillerId,
    updateZip: updateZip,
    updateCity: updateCity,
    updateState: updateState
  }
)(withStyles(styles)(PostsIndex));
