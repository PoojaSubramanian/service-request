...
class PostsShow extends React.Component {
  ...
  onOffer() {
    ...
    $.ajax({
      url: `${config.SERVICE_DATABASE_URL}/service/${post._id}?fulfillerName=${
        this.props.userName
      }&fulfillerId=${this.props.userId}&status=Offered`,
      type: "GET",
      success: data => {
  ...
  onAccept() {
    ...
    $.ajax({
      url: `${config.SERVICE_DATABASE_URL}/service/${
        post._id
                ...
                <Typography variant="subtitle1">
                  {this.props.userId === comment.userId && (
                    <IconButton
                      ...
                      onClick={this.OndeleteComment.bind(commentServiceIds)}
                    >
                      <DeleteIcon />
                    </IconButton>
                  )}
    ...
    return (
      <div>
            ...
            <Grid item xs>
              <SnackbarContent
                ...
                message={
                  <div>
                    <h4>
                      {post.userName} {post.lastName ? post.lastName : ""}
                    </h4>
                    <i>{moment(timeFromDb).fromNow()}</i>
                    <h3>{post.subject}</h3>
                    <p>{post.text}</p>
                    <i>
                      Status : {post.status} by {post.fulfillerName}
                    </i>{" "}
                    ...
                    {post.status === "Fulfillment In Progress" &&
                      this.props.userId === this.props.post.userId && (
                        <button
                          type="submit"
                          className="btn btn-success pull-xs-right"
                          onClick={this.onFulfill.bind(this)}
                        >
                          {" "}
        ...
        <button
          name="comment"
          type="submit"
          onClick={handleSubmit(this.onAddComment.bind(this))}
...
export default reduxForm({ form: "PostsCommentsForm" })(
  connect(
    mapStateToProps,
    { fetchPost, fetchComments, deletePost, deleteComment }
  )(withStyles(styles)(PostsShow))
);
